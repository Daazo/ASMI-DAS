...
async def reaction_role_setup(
    ...
    try:
        ...
        await log_action(interaction.guild.id, "reaction_role", f"🎭 [REACTION ROLE] Setup by {interaction.user} - {emoji} → {role.name}")
            ...
            if give_role and member:
                try:
                    ...
                    if give_role in member.roles:
                        ...
                        await log_action(guild.id, "reaction_role", f"🎭 [REACTION ROLE] {give_role.name} removed from {member}")
                    ...
                    if remove_role_enabled and remove_role_id:
                        ...
                        if remove_role and remove_role not in member.roles:
                            ...
                            await log_action(guild.id, "reaction_role", f"🎭 [REACTION ROLE] {remove_role.name} restored to {member}")
