...
async def timeout_user(member, guild, duration_minutes, reason, message_content, offense_count):
    ...
    try:
        ...
        try:
            ...
            dm_embed.add_field(
                name="⚠️ **Warning**",
                value="Repeated offenses will result in longer timeouts. Please follow server rules.",
                inline=False
            )
            dm_embed.set_footer(text="ᴠᴀᴀᴢʜᴀ Auto-Moderation", icon_url=bot.user.display_avatar.url)
            await member.send(embed=dm_embed)
        except:
            pass  # User has DMs disabled
        
        # Log the timeout
        log_message = f"⏰ [AUTO-TIMEOUT] {member} | Reason: {reason} | Duration: {duration_minutes}m | Offense: #{offense_count}"
        if message_content and len(message_content) < 100:
            log_message += f" | Message: \"{message_content[:50]}...\""
        
        await log_action(guild.id, "moderation", log_message)
        ...
        embed.set_footer(text="🤖 ᴠᴀᴀᴢʜᴀ Auto-Moderation")
        ...
        if 'moderation' in log_channels:
            notification_channel = bot.get_channel(int(log_channels['moderation']))
        elif 'all' in log_channels:
            notification_channel = bot.get_channel(int(log_channels['all']))
...
async def remove_timeout(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        embed.set_footer(text="🤖 ᴠᴀᴀᴢʜᴀ Moderation", icon_url=bot.user.display_avatar.url)
        ...
        try:
            ...
            dm_embed.set_footer(text="ᴠᴀᴀᴢʜᴀ Moderation", icon_url=bot.user.display_avatar.url)
...
async def timeout_stats(interaction: discord.Interaction, user: discord.Member = None):
    ...
    if guild_id in user_messages and user_id in user_messages[guild_id]:
        ...
        embed.set_footer(text="🤖 ᴠᴀᴀᴢʜᴀ Auto-Moderation Stats", icon_url=bot.user.display_avatar.url)
    else:
        embed = discord.Embed(
            title=f"📊 **Timeout Stats for {user.display_name}**",
